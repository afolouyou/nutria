defmodule NutriaWeb.ProfileLive.Index do
  ...
  defp initial(user), do: user && user.name && user.name != "" && String.first(user.name) || "?"
      ...
      <div class="profile-body">
        <div class="profile-inner">
          ...
          <div class="settings-profile-card">
            ...
            <form phx-change="validate_avatar" phx-submit="save_avatar" phx-drop-target={@uploads.avatar.ref}>
              <.live_file_input upload={@uploads.avatar} class="hidden" />
              <button type="button" onclick={"document.getElementById('#{@uploads.avatar.ref}').click()"} class="settings-theme-row" style="justify-content:center">
                Trocar foto
              </button>
              <button type="submit" class="settings-theme-row" style="justify-content:center;background:var(--accent-soft);color:var(--accent)">
                Salvar
              </button>
            </form>
          </div>
